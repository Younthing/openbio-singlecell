...
GENCODE_REFERENCE = AnalysisReference(
    citation=(
        "Frankish A et al. GENCODE 2025: reference gene annotation for human and mouse. "
...
def _expression_snapshot_audit(matrix: Any) -> dict[str, Any]:
    ...
    finite_integer_like = bool(
        ...
        or science.np.allclose(
            finite_values,
            science.np.rint(finite_values),
            rtol=0.0,
            atol=1e-8,
        )
    )
...
def _require_in_memory_adata(adata: AnnData, *, operation: str) -> None:
    if bool(adata.isbacked):
        raise ValueError(f"{operation} requires an in-memory AnnData; call adata.to_memory() first.")


def _copy_adata_allowing_duplicate_names(adata: AnnData) -> AnnData:
...
def _string_label_collision(values: Any, *, label: str) -> None:
    ...
    for value in values:
        if bool(dependencies.require_scientific_dependencies().pd.isna(value)):
            continue
        key = str(value)
...
def _bounded_value_counts(values: Any, limit: int = 64) -> dict[str, Any]:
    ...
    return {
        ...
        "truncated": total_categories > limit,
    }
        ...
        def subset_observations(adata):
            ...
            for value in series:
                ...
                if not any(_equal_values(value, existing) for existing in representatives):
                    representatives.append(value)
            collisions = [
                key for key, representatives in rendered.items() if len(representatives) > 1
            ]
            if collisions:
                raise ValueError(
...
def _source_provenance(adata: AnnData) -> Mapping[str, Any] | None:
...
def _verify_shared_provenance(adata: AnnData, subset_adata: AnnData) -> str:
...
def _merge_annotation_values(
    ...
    *,
    ...
    source_ordered = (
        bool(source.cat.ordered) if isinstance(source.dtype, science.pd.CategoricalDtype) else None
    )
    ...
    target_before_dtype = str(adata.obs[target_column].dtype) if target_exists else None
    if target_exists:
        target = adata.obs[target_column].copy()
    else:
        target = science.pd.Series(science.pd.NA, index=adata.obs_names, dtype="object")
    ...
    output = _copy_adata_allowing_duplicate_names(adata)
    dtype_fallback_to_object = False
    source_is_categorical = isinstance(source.dtype, science.pd.CategoricalDtype)
    target_is_categorical = target_exists and isinstance(
        adata.obs[target_column].dtype,
        science.pd.CategoricalDtype,
    )
    if not target_exists:
        ...
        categories = list(adata.obs[target_column].cat.categories)
        ...
        ordered = bool(
            adata.obs[target_column].cat.ordered
            and source.cat.ordered
            and adata.obs[target_column].cat.categories.equals(source.cat.categories)
        )
    ...
    statistics = {
        ...
        "target_categorical_ordered_before": (
            bool(adata.obs[target_column].cat.ordered) if target_is_categorical else None
        ),
...
def _merge_observation_annotations_code(
    ...
    return dedent(
        ...
        def _source_provenance(value):
            metadata = value.uns.get("openbio_singlecell")
            if not isinstance(metadata, Mapping):
                return None
            source = metadata.get("source")
            return source if isinstance(source, Mapping) and source else None
        ...
        def merge_observation_annotations(adata, subset_adata):
            ...
            target_present = target.notna()
            fill_mask = source_present & ~target_present
            identical_mask = pd.Series(False, index=adata.obs_names)
            conflict_mask = pd.Series(False, index=adata.obs_names)
            for name in adata.obs_names[source_present & target_present]:
                if _equal_values(aligned.loc[name], target.loc[name]):
                    identical_mask.loc[name] = True
                else:
                    conflict_mask.loc[name] = True
            conflict_count = int(conflict_mask.sum())
            ...
            target_is_categorical = target_exists and isinstance(
                adata.obs[target_column].dtype, pd.CategoricalDtype
            )
            if not target_exists:
                output.obs[target_column] = aligned.copy()
...
def _bounded_gtf_headers(headers: Mapping[str, str], *, limit: int = 32) -> dict[str, str]:
    return {
        ...
        for key, value in list(headers.items())[:limit]
    }
...
def _validate_gtf_identity_inputs(
    ...
    *,
    ...
    science = dependencies.require_scientific_dependencies()
    _require_in_memory_adata(adata, operation="GTF identity annotation")
    ...
    if id_source == "var_column" and ORIGINAL_FEATURE_NAMES_COLUMN in adata.var:
        raise ValueError(
            f"Reserved original feature-name annotation already exists: {ORIGINAL_FEATURE_NAMES_COLUMN!r}"
        )

    source_series = (
        science.pd.Series(adata.var_names, index=adata.var_names)
        if id_source == "var_names"
        else adata.var[id_column]
    )
    ...
    duplicate_count = len(gene_ids) - len(set(gene_ids))
    normalized_ids = [_stable_gene_id(value) for value in gene_ids]
    normalized_duplicate_count = len(normalized_ids) - len(set(normalized_ids))
    ...
    if gene_name_column in adata.var:
        non_string = [
            ...
            if not (
                isinstance(science.pd.isna(value), (bool, science.np.bool_))
                and bool(science.pd.isna(value))
            )
            and not isinstance(value, str)
        ]
        existing_non_string_count = len(non_string)
    return id_column, gene_name_column, gene_ids, normalized_ids, {
        "duplicate_stable_id_occurrences": duplicate_count,
        "version_normalized_id_collisions": normalized_duplicate_count,
...
def _gtf_identity_operation(
    ...
    *,
    ...
    for position, mapped_name in enumerate(mapped_names):
        ...
        current_is_missing = bool(missing_result) if isinstance(
            missing_result,
            (bool, science.np.bool_),
        ) else False
        if current_is_missing:
            ...
            conflicts.append(
                f"{gene_ids[position]!r}: existing={str(current)!r}, GTF={mapped_name!r}"
            )
    output = _copy_adata_allowing_duplicate_names(adata)
    ...
    output.var[GTF_MAPPED_COLUMN] = science.np.asarray([value is not None for value in mapped_names], dtype=bool)
    ...
    for value in final_symbols:
        if not any(_equal_annotation_values(value, existing) for existing in unique_symbols):
            unique_symbols.append(value)
    symbol_duplicate_occurrences = len(final_symbols) - len(unique_symbols)
    statistics = {
        ...
        "duplicate_gene_symbol_occurrences": symbol_duplicate_occurrences,
        **identity_audit,
    }
    return output, statistics
...
class OpenBioSingleCellSnapshotExpression(io.ComfyNode):
    ...
    def execute(
        ...
        science = dependencies.require_scientific_dependencies()
        _require_in_memory_adata(adata, operation="Raw snapshot creation")
        ...
        counts = matrix.copy()
        output = _copy_adata_allowing_duplicate_names(adata)
        output.layers[CANONICAL_COUNTS_LAYER] = counts.copy()
        raw_source = _copy_adata_allowing_duplicate_names(output)
        raw_source.X = counts.copy()
        output.raw = raw_source
        parameters = {
        ...
        report, code = make_analysis_report(
            ...
            key_results={
                ...
                "total_expression": (
                    expression_audit["finite_expression_sum"]
                    if expression_audit["nonfinite_value_count"] == 0
                    else None
                ),
...
class OpenBioSingleCellSubsetObservations(io.ComfyNode):
    ...
    def execute(
        ...
        _require_in_memory_adata(adata, operation="Observation subsetting")
        column = column.strip()
        if not column:
            raise ValueError("Observation annotation name cannot be empty.")
        if column not in adata.obs:
        ...
        started_at = time.perf_counter()
        cells, genes = int(adata.n_obs), int(adata.n_vars)
        output = _copy_adata_allowing_duplicate_names(adata[mask.to_numpy(), :])
        available = set(str(value) for value in rendered[not_missing].unique())
        ...
        output_counts = {
            value: int((output_rendered == value).fillna(False).sum()) for value in selected_values
        }
        parameters = {
            "column": column,
...
class OpenBioSingleCellMergeObservationAnnotations(io.ComfyNode):
    ...
    def execute(
        ...
        _require_in_memory_adata(adata, operation="Observation annotation transfer")
        _require_in_memory_adata(subset_adata, operation="Observation annotation transfer")
        ...
        if provenance_status == "not_available":
            ...
            warnings_list.append(
                ...
                "used for the requested transfer; confirm that the shared names refer to the intended cells."
            )
        if len(source_only):
            warnings_list.append(
        ...
        if not bool(subset_adata.obs[source_column].notna().any()):
            warnings_list.append(
                "The source annotation contains no non-missing values; no target values were transferred."
            )
        ...
        source_missing = statistics["source_missing"]
...
class OpenBioSingleCellMapGeneIdsFromGTF(io.ComfyNode):
    ...
    def execute(
        ...
        id_source: str = "var_column",
        id_column: str = "gene_ids",
        gene_name_column: str = "gene_symbols",
    ) -> io.NodeOutput:
        if id_source not in {"var_names", "var_column"}:
            raise ValueError(f"Unsupported gene ID source: {id_source!r}")
        id_column = id_column.strip()
        ...
        provenance = input_file_provenance(gtf_path, GTF_EXTENSIONS)
        ...
        if adata.raw is not None:
            limitations.append(
                "Raw is an independent snapshot and retained its original feature annotations and index."
            )
        parameters = {
            "gtf_path": gtf_path,
            "id_source": id_source,
        ...
        report, code = make_analysis_report(
            ...
            key_results={
                **statistics,
                "mapped_fraction": (
                    float(statistics["mapped_features"] / genes) if genes else None
                ),
                "id_source": id_source,
