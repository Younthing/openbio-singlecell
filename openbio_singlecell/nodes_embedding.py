...
class OpenBioSingleCellPCA(io.ComfyNode):
    ...
    def execute(
        ...
        return io.NodeOutput(output, summary, code)
...
class OpenBioSingleCellNeighbors(io.ComfyNode):
    ...
    def execute(
        ...
        code = code.replace("from collections.abc import Mapping", "import warnings\nfrom collections.abc import Mapping", 1)
        code = code.replace(
            'temp_key = "__openbio_neighbors_representation"',
...
class OpenBioSingleCellTSNE(io.ComfyNode):
    ...
    def execute(
        ...
        summary, code = make_analysis_report(
            ...
            warnings=[
                *(
                    ["t-SNE perplexity is below 1; this is executable but unusually local."]
                    if perplexity < 1
                    else []
                ),
                *(
...
def run_leiden(adata):
    ...
    for offset in range({repeats!r}):
        ...
        suffix = 0
        while offset and (key in target.obs or key in target.uns):
            suffix += 1
            key = f"__openbio_leiden_stability_{{offset}}_{{suffix}}"
        sc.tl.leiden(target, adjacency=adjacency, resolution={resolution!r}, key_added=key, random_state={seed!r} + offset, flavor="igraph", n_iterations={iterations!r}, directed=False, use_weights=True, objective_function="modularity")
    ...
    def execute(
        ...
        summary, code = make_analysis_report(
            ...
            warnings=[
                ...
                *(
                    ["Leiden stability was not assessed because stability_repeats=1."]
