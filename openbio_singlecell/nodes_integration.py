...
from .scvi_model import SCVI_GLOBAL_RNG_LOCK, SCVIModel
...
def _require_harmonypy_200() -> tuple[Any, str]:
    try:
        ...
        raise RuntimeError(
            "Harmony Integration requires a compatible harmonypy 2.0.x installation."
        ) from error
...
def _validate_scvi_counts(
    ...
    nonzero = (
        int(matrix.copy().count_nonzero()) if science.sparse.issparse(matrix) else int(science.np.count_nonzero(matrix))
    )
        ...
        def leiden_resolution_sweep(adata):
            ...
            for resolution, key in zip(resolutions, keys, strict=True):
                ...
                for offset in range(stability_repeats):
                    target = output if offset == 0 else output.copy()
                    run_key = key
                    if offset:
                        run_key = f"__openbio_leiden_stability_{{offset}}"
                        collision = 0
                        while run_key in target.obs or run_key in target.uns:
                            collision += 1
                    ...
                    membership, run_sizes, run_modularity = validated_partition(target, run_key, resolution)
...
def _harmony_code(parameters: Mapping[str, Any]) -> str:
    return dedent(
        ...
        def run_harmony_integration(adata):
            ...
            if not np.array_equal(np.asarray(output.obsm[basis]), original_basis):
                raise RuntimeError("Harmony backend execution changed the original basis unexpectedly.")
...
class OpenBioSingleCellHarmonyIntegration(io.ComfyNode):
    ...
    def execute(
        ...
        with python_warnings.catch_warnings():
            ...
            output = adata.copy()
        original_basis = science.np.asarray(basis_values).copy()
        ...
        output.obsm[adjusted_basis] = corrected
        original_basis_preserved = bool(science.np.array_equal(science.np.asarray(output.obsm[basis]), original_basis))
        if not original_basis_preserved:
            raise RuntimeError("Harmony backend execution changed the original basis unexpectedly.")
        ...
        report, code = make_analysis_report(
            ...
            key_results={
                ...
                "original_basis_preserved": original_basis_preserved,
    ...
    def execute(
        ...
        warnings = [
            *count_warnings,
            ...
            "The trained scVI model is process-local; saved workflows do not contain its fitted weights.",
        ...
        trained_model = SCVIModel(model, training_adata, parameters, diagnostics=diagnostics)
        report, code = make_analysis_report(
            node_id="OpenBioSingleCellSCVIIntegration",
            ...
            results=(
                f"A trained process-local model and finite {cells:,} by {n_latent:,} latent representation were produced. "
        ...
        return io.NodeOutput(output, trained_model, report, code)
...
class OpenBioSingleCellLeidenResolutionSweep(io.ComfyNode):
    ...
    def execute(
        ...
        if total_runs > _LEIDEN_SWEEP_WORKLOAD_WARNING:
            warnings.append(
                f"Leiden sweep requests {total_runs} backend runs; execution may be resource intensive."
            )
        ...
        if any(
            row["stability_min_ari"] is not None and row["stability_min_ari"] < 0.9 for row in rows
        ):
            warnings.append(
