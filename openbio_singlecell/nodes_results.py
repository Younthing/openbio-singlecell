...
class OpenBioSingleCellMarkerGenes(io.ComfyNode):
    ...
    def execute(
        ...
        return io.NodeOutput(table_result, universe_result, report, code)
...
class OpenBioSingleCellUMAPPlot(io.ComfyNode):
    ...
    def execute(
        ...
        uses_scanpy_metadata = bool(
            provenance["scanpy_metadata_verified"] or provenance["openbio_history_verified"]
        )
...
class OpenBioSingleCellFilterMarkerGenes(io.ComfyNode):
    ...
    def execute(
        ...
        input_by_group = {group: int((table.table["group"] == group).sum()) for group in provenance["group_labels"]}
        ...
        report, code = make_analysis_report(
            ...
            methods=(
                ...
                "deterministic pandas mask using inclusive "
                "comparisons: log2_fold_change_approx >= min_log2_fold_change, fraction_in_group >= "
                "min_fraction_in_group, fraction_reference <= max_fraction_reference, and p_adjusted <= "
                "max_p_adjusted. Upstream ranks and row order were preserved. This differs from Scanpy's strict "
                "AnnData-mutating filter helper."
            ),
            results=(
        ...
        return io.NodeOutput(filtered, universe, report, code)
...
class OpenBioSingleCellPCAMetadataAssociations(io.ComfyNode):
    ...
    def execute(
        ...
        code = _pca_metadata_associations_code(
            use_rep,
            sample_key,
            categorical_keys,
            continuous_keys,
        ...
        report, code = make_analysis_report(
            ...
            methods=(
                ...
                "categorical metadata were required to be constant within each Sample and screened with standard "
                "one-way ANOVA, with eta squared reported as effect size. Benjamini-Hochberg correction was applied "
                "once across every valid PC-by-metadata hypothesis."
            ),
            results=(
